...
class AgentVRP():
    ...
    def __init__(self, input):
        ...
        self.time_windows=tf.cast(input[3], tf.string)
        self.time_windows=tf.concat([tf.zeros((self.time_windows.shape[0],1), dtype=tf.string),self.time_windows],1)
        self.service_time=tf.cast(input[4], tf.float32)
    ...
    def get_mask(self):
        ...
        if self.i>0:
            for i in range(self.coords.shape[1]):
                ...
                temp_dist=tf.math.scalar_mul(2,tf.reduce_sum(tf.norm(var[:, 1:] - var[:, :-1], ord=2, axis=2), axis=1),
                                             name=None)
                temp_service_time=tf.reshape(self.service_time[:,i],(self.batch_size,1))#???????
                temp_time=tf.add(tf.reshape(temp_dist,(self.batch_size,1)),self.temp_tour_length,temp_service_time)
                temp_time_windows=self.time_windows
                L=[]
                for j in range(self.batch_size):
                    a=temp_time_windows[j][i].numpy()
                    if len(a)>=1:
                        min_t=int(a[1:2])
                        max_t=int(a[4:5])
                        L.append(not ((temp_time[j,:].numpy()[0]>=min_t ) & (max_t>= temp_time[j,:].numpy()[0])))
                    else:
                        L.append(False)
    ...
    def step(self, action):
        ...
        if self.i  > 0:
            pi=tf.concat([self.prev_a,selected], 1)
            #print(self.get_route_length(pi))
            travel_time=tf.reshape(tf.math.scalar_mul(2, self.get_route_length(pi), name=None),(self.batch_size,1))
            d =tf.gather(self.service_time, tf.cast(selected, tf.int32), batch_dims=1)
            self.temp_tour_length =tf.add(d,travel_time)
...
####+ tf.norm(d[:, 0] - self.coords[0], ord=2, axis=1) A ne pas oublier 
